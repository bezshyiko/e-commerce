    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".collections__slider")) new core(".collections__slider", {
                ...
                speed: 800,
                pagination: {
                    el: ".slider-collections__pagination",
                    clickable: true
                },
                navigation: {
                    prevEl: ".collections-nav__prev",
                    nextEl: ".collections-nav__next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
            ...
            if (document.querySelector(".slider-for-you")) new core(".slider-for-you", {
                modules: [ Navigation, Pagination, Autoplay ],
                observer: true,
                observeParents: true,
                spaceBetween: 2,
                ...
                navigation: {
                    prevEl: ".slider-for-you__prev",
                    nextEl: ".slider-for-you__next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1.5,
            ...
            const mainSliderProduct = new core(".main-slider-product", {
            ...
            const previewSlider = new core(".preview-slider-product", {
                modules: [ Navigation, Pagination, Autoplay ],
                observer: true,
                observeParents: true,
                spaceBetween: 10,
            ...
            mainSliderProduct.params.control = previewSlider;
            previewSlider.params.control = mainSliderProduct;
            document.addEventListener("click", (e => {
                document.querySelectorAll("[data-slide]");
                let sliders = document.querySelector("[data-sliders]");
                let closeSlidersBtn = document.querySelector("[data-close-sliders]");
                const targetElement = e.target;
                if (targetElement.closest("[data-slide]")) {
                    sliders.classList.add("fullscreen");
                    bodyLock();
                    setTimeout((function() {
                        mainSliderProduct.update();
                        previewSlider.update();
                    }), 200);
                    return;
                }
                if (targetElement === closeSlidersBtn && sliders.classList.contains("fullscreen")) {
                    sliders.classList.remove("fullscreen");
                    bodyUnlock();
                }
            }), false);
        }
        ...
        document.addEventListener("click", pageNavigationHeader);
        function pageNavigationHeader(e) {
            const targetElement = e.target;
            if (targetElement.closest("[data-parent]")) {
                const buttonCategoriesId = targetElement.dataset.parent;
                const productsMenuId = document.querySelector(`[data-menu = "${buttonCategoriesId}"]`);
                const decor = targetElement.closest("[data-parent]").nextElementSibling;
                if (productsMenuId) {
                    let activeMenu = document.querySelector("._products-menu-open");
                    let activeDecor = document.querySelector("._active-decore");
                    if (activeMenu !== targetElement) {
                        activeMenu.classList.remove("_products-menu-open");
                        activeDecor.classList.remove("_active-decore");
                    }
                    productsMenuId.classList.add("_products-menu-open");
                    decor.classList.add("_active-decore");
        ...
        document.addEventListener("click", (e => {
            const targetElement = e.target;
            if (targetElement.closest("[data-view]")) {
                const buttonView = document.querySelector("[data-view]");
                const catalogView = document.querySelector("[data-view-catalog]");
                if (buttonView.classList.contains("_list") && catalogView.classList.contains("_list")) {
                    buttonView.classList.remove("_list");
                    catalogView.classList.remove("_list");
                    buttonView.classList.add("_full");
                    catalogView.classList.add("_full");
                } else if (buttonView.classList.contains("_full") && catalogView.classList.contains("_full")) {
                    buttonView.classList.remove("_full");
                    catalogView.classList.remove("_full");
                } else {
                    buttonView.classList.add("_list");
                    catalogView.classList.add("_list");
                }
            }
        }));
        const catalogBlock = document.querySelector("[data-catalog]");
        const catalogItems = document.querySelectorAll("[data-catalog-item]");
        if (catalogBlock) {
            document.addEventListener("click", pageFilterAction);
            function pageFilterAction(e) {
                const targetElement = e.target;
                if (targetElement.closest("[data-filter]")) {
                    const catalogItemValue = targetElement.dataset.filter;
                    const buttonFilterActive = document.querySelector("._active-filter");
                    const removeCatalogItems = function(catalog) {
                        while (catalog.firstChild) catalog.removeChild(catalog.firstChild);
                    };
                    const updateCatalogItems = function(catalog, items) {
                        removeCatalogItems(catalog);
                        for (let i = 0; i < items.length; i += 1) catalog.appendChild(items[i]);
                    };
                    if (buttonFilterActive !== targetElement) {
                        buttonFilterActive.classList.remove("_active-filter");
                        targetElement.classList.add("_active-filter");
                    }
                    if ("All" === catalogItemValue) updateCatalogItems(catalogBlock, catalogItems); else {
                        let filteredItems = [];
                        for (let currentItems of catalogItems) if (currentItems.dataset.catalogItem === catalogItemValue) filteredItems.push(currentItems);
                        updateCatalogItems(catalogBlock, filteredItems);
                    }
                    e.preventDefault();
                }
            }
        }
        const bodyFilters = document.querySelector("[data-filters-body]");
        if (bodyFilters) document.addEventListener("click", (e => {
            const targetElement = e.target;
            const openButton = targetElement.closest("[data-open-button]");
            const closeButton = targetElement.closest("[data-close-button]");
            function openFilters() {
                bodyLock();
                bodyFilters.classList.add("open-filters");
                document.documentElement.classList.add("filters-show");
            }
            function closeFilters() {
                bodyUnlock();
                bodyFilters.classList.remove("open-filters");
            ...
            if (closeButton || !targetElement.closest("[data-filters-body]") && bodyFilters.classList.contains("open-filters")) {
                e.preventDefault();
                closeFilters();
            }
        }));
        const cartDOMElement = document.querySelector("[data-cart]");
        const totalPriceDOMElement = document.querySelector("[data-total-price]");
        const totalPriceInputDOMElement = document.querySelector("[data-total-input-price]");
        const iconCartQuantity = document.querySelector("[data-cart-icon-quantity]");
        document.querySelector("[data-cart-order-form]");
        if (cartDOMElement) {
            const cart = JSON.parse(localStorage.getItem("cart")) || {};
            const generateID = (str1, str2) => {
                const secondParam = str2 ? `-${str2}` : "";
                return `${str1}${secondParam}`;
            };
            const getProductData = productDomElement => {
                const name = productDomElement.dataset.productName;
                const price = productDomElement.dataset.productPrice;
                const attribute = productDomElement.dataset.productAttribute;
                const size = productDomElement.dataset.productSize;
                const quantity = 1;
                const src = productDomElement.dataset.productSrc;
            ...
            const renderCart = () => {
                ...
                ids.forEach((id => {
                    renderCartItem(cart[id]);
                }));
            };
            const changeTotalProducts = () => {
                const totalProducts = Object.keys(cart).reduce(((acc, id) => {
            ...
            const changeTotalPrice = () => {
                ...
                if (totalPriceDOMElement) totalPriceDOMElement.textContent = totalPrice;
                if (totalPriceInputDOMElement) totalPriceInputDOMElement.value = totalPrice;
            };
            const flyToCart = productElem => {
                ...
                if (!addToCartBtn.classList.contains("_fly")) {
                    ...
                    const productImage = itemProduct.querySelector("[data-cart-image]");
            ...
            const renderCartItem = ({name, price, attribute, quantity, id, src, size}) => {
            ...
            const cartInit = () => {
                ...
                document.addEventListener("click", (e => {
                    const targetElement = e.target;
                    if (targetElement.hasAttribute("data-add-to-cart")) {
                        e.preventDefault();
                        const productDomElement = targetElement.closest("[data-product]");
                        const data = getProductData(productDomElement);
                        addCartItem(data);
                        flyToCart(productDomElement);
                    }
                    if (targetElement.hasAttribute("data-cart-delete")) {
                        e.preventDefault();
                        const cartItemDOMElement = targetElement.closest("[data-product-id]");
                        const productID = cartItemDOMElement.dataset.productId;
                        deleteCartItem(productID);
                    }
                    if (targetElement.hasAttribute("data-cart-quantity-plus")) {
                        e.preventDefault();
                        const cartItemDOMElement = targetElement.closest("[data-product-id]");
                        const productID = cartItemDOMElement.dataset.productId;
                        increaseQuantity(productID);
                    }
                    if (targetElement.hasAttribute("data-cart-quantity-minus")) {
                        e.preventDefault();
                        const cartItemDOMElement = targetElement.closest("[data-product-id]");
                        const productID = cartItemDOMElement.dataset.productId;
